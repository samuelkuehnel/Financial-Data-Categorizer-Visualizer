...
def sum_expenses(classes, data, month, year):
    ...
    for item in classes:
        ...
        mask_1 = data["Name Zahlungsbeteiligter"].str.contains(
            classes[item],
            case=False).fillna(False)
        mask_2 = data["Verwendungszweck"].str.contains(
            classes[item],
            case=False).fillna(False)
...
def finances(expenses_fix, expenses_variable, income, data, year):
    ...
    data["Betrag"] = data["Betrag"].map(
        lambda a: float(np.char.replace(a, ",", ".")))
    # Monat herausfinden
    ...
    result_expenses_fix, neg_data = sum_expenses(
        expenses_fix, neg_data, month, year)
    result_expenses_variables, neg_data = sum_expenses(
        expenses_variable, neg_data, month, year)
    ...
    sum_classes = {"nicht zugeordnet": np.round(np.sum(
        data["Betrag"]), decimals=2), "total income": total_income,
...
def summarize_year(year, expenses_fix, expenses_variable, income):
    ...
    summarized.update(
        dict(zip(expenses_variable, np.zeros(len(expenses_variable)).T)))
